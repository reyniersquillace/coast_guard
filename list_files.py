        ...
        select = db.select([db.files, 
                    ...
                    from_obj=[db.files.\
                        outerjoin(db.obs,
                            onclause=(db.files.c.obs_id ==
                                        db.obs.c.obs_id))]).\
                    where(db.files.c.file_id.in_(file_ids)).\
                    order_by(db.files.c.added.asc())
        result = conn.execute(select)
        rows = result.fetchall()
        result.close()
...
@click.option('--type', 'type', type=str,
                help='Type of files to list. Options are:'
                     '%s' % sorted(set(FILETYPE_TO_WHERE.keys())))
@click.option('-F', '--file-id', 'file_ids', multiple=True, default=(),
...
@click.option('--sort', 'sortkeys', metavar='SORTKEY', multiple=True,
                ...
                help="DB column to sort raw data files by. Multiple "
                    ...
                    "by 'added'.)")
@click.option("--fmt", "fmt", default=None,
                help="Write custom format for each matching file.")
@cli_common.standard_options
@cli_common.debug_options
def main(psrnames, type, file_ids, sortkeys, fmt):
    """List files generated by the automated pipeline."""
    if file_ids:
        rows = get_files_by_id(list(file_ids))
    else:
        rows = get_files(list(psrnames), type)
    info = {}

    utils.sort_by_keys(rows, list(sortkeys))
    for row in rows:
        if fmt is not None:
            print(fmt.encode().decode('unicode_escape') % row)
        else:
